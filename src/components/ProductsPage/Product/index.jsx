            ...
            <NavLink exact to={`/product-page/${data.id}`}>
                <div className={style.item}>
                    ...
                    <div className={style.size}>
                        {data.size.map(p => <button onClick={() => setSize(p)}
                                                    style={{border: size === p && "2px solid #8855DD"}}>{p}</button>)}
                    </div>
                    <div className={style.colors}>
                        {data.colors.map(c => <button onClick={() => setColor(c)}
                                                      style={{border: color === c && "2px solid #8855DD"}}>
                    ...
                    <div className={style.price}>
                        ...
                        {data.discount ?
                            <div><p className={style.discountPrice}>{data.price} сом</p><p>{priceDis} сом</p></div> :
                            <p>{data.price} сом</p>}
                    </div>
